...
<html>
 ...
 echo getJG(0,0,"",$_GET['typeid'],$_GET['channel']); ?>
                 ...
                 <!-- Date range -->
                  ...
                  <?php
 $channelfield = M("Channelfield")->where("chid=".$_GET['channel'])->order("sort")->select(); foreach ($channelfield as $v) { ?>
                    <div class="form-group">
                      ...
                      <?php }elseif ($v['typeid'] == "6" ) { $tmp1 = ''; $tmps = explode(',', $v['fielddefault']); foreach ($tmps as $tm) { $tmp1.= '<option value="'.$tm.'">'.$tm.'</option>'; } ?>
